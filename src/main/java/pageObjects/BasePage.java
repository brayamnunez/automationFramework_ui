...
public class BasePage extends DriverFactory {
    ...
    public void clickOnDynamicDropdownListByText(String dropDownToClick, String textToSearch) {
        Select dynamicDropdown = new Select(driver.findElement(By.xpath(dropDownToClick)));
        dynamicDropdown.selectByVisibleText(textToSearch);
    }

    public String replaceValue(String xpath, String replacementToken, String value) {
        return xpath.replace(replacementToken, value);
    }

    public WebElement getWebElementObjectBy(By by) {
    ...
    public void actionMoveByElement(WebElement element) {
        Actions ob = new Actions(driver);
        try {
            this.wait.until(ExpectedConditions.elementToBeClickable(element));
            ob.moveToElement(element).build().perform();
            System.out.println("Successfully Action Moved on the WebElement, using locator: " + "<" + element.toString() + ">");
        } catch (Exception e) {
            System.out.println("Unable to Action Move and Click on the WebElement, using locator: " + "<" + element.toString() + ">");
            Assert.fail("Unable to Action Move and Click on the WebElement, Exception: " + e.getMessage());
        }
    ...
    public void scrollToElementByLocator(By by) {
        WebElement element = driver.findElement(by);
        try {
            this.wait.until(ExpectedConditions.visibilityOf(element)).isEnabled();
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", element);
    ...
    public void scrollToWebElementByWebElementLocatorAndClick(WebElement elementToScrollAndClick){
        try {
            this.wait.until(ExpectedConditions.visibilityOf(elementToScrollAndClick)).isEnabled();
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", elementToScrollAndClick);
            ((JavascriptExecutor) driver).executeScript("window.scrollBy(0, -500)");
            elementToScrollAndClick.click();
            System.out.println("Successfully scrolled to the WebElement, using locator: " + "<" + elementToScrollAndClick.toString() + ">");
        } catch (Exception e) {
            System.out.println("Unable to scroll to the WebElement, using locator: " + "<" + elementToScrollAndClick.toString() + ">");
            Assert.fail("Unable to scroll to the WebElement, Exception: " + e.getMessage());
        }
    }
    ...
    public boolean verifyVisibilityOfPopUp(WebElement elementToWait, WebElement elementToClick) {
        try {
            this.wait.until(ExpectedConditions.visibilityOf(elementToWait));
            elementToClick.click();
            System.out.println("WebElement is visible using locator: " + "<" + elementToClick.toString() + ">");
            return true;
        } catch (Exception e) {
            System.out.println("WebElement is NOT visible, using locator: " + "<" + elementToWait.toString() + ">");
            return false;
        }
    }
    ...
    public void waitUntilWebElementIsVisibleAndClick(WebElement elementToClick) {
        wait.until(ExpectedConditions.visibilityOf(elementToClick)).click();
    }

    public void clickAndSearchElementFromDropdownList(List<WebElement> autoCompleteOptionList, String elementToSelect) {
        try {
            for (WebElement optionToSelect : autoCompleteOptionList) {
                if (optionToSelect.getText().equals(elementToSelect)) {
                    System.out.println("Trying to select: " + elementToSelect);
    ...
    public static String returnScreenshotName() {
        return (System.getProperty("user.dir") + "\\output\\imgs\\" + screenShotName);
    }
